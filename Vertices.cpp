...
namespace {
	...
	// 位置情報とクスチャ座標
	const D3D11_INPUT_ELEMENT_DESC VertexPositionTexture_InputElementDescs[2] = {
		{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0,							   D3D11_INPUT_PER_VERTEX_DATA, 0 },
...
// インプットレイアウトの配列の取得
const D3D11_INPUT_ELEMENT_DESC* VertexPositionTexture::GetInputElementDescs()
{
	return VertexPositionTexture_InputElementDescs;
}

// インプットレイアウトの配列の要素数の取得
UINT VertexPositionTexture::GetInputElementDescsLength()
{
	return _countof(VertexPositionTexture_InputElementDescs);
}
